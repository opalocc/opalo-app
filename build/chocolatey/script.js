...
const updateInstallScript = async () => {
    const checksum = await calculateChecksum(path.join(__dirname,'..','..', 'out','make','squirrel.windows','x64',`opalo-${packageJson.version} Setup.exe`))
...
const updateNuspec = async () => {
    await replaceInFile(path.join(__dirname, 'opalo.nuspec'), /__REPLACE_VERSION__/g,packageJson.version);
};
